...
后面两个公式一共出现五个希腊字母，先一次说清——每一个都不复杂，复杂的是它们组合出来的效果。

| 符号 | 读作 | 是什么 | 怎么来的 |
|---|---|---|---|
| $\mu$ | 缪 (mu) | 均值（mean） | $\mu = \text{mean}(x)$，向量所有元素的平均数 |
| $\sigma^2$ | 西格玛平方 (sigma) | 方差（variance） | $\sigma^2 = \text{mean}\left((x - \mu)^2\right)$，每个元素离均值有多远，再取平均 |
| $\epsilon$ | 伊普西龙 (epsilon) | 一个极小的数，如 1e-5 | 直接写死的常数，防止除以 0 |
| $\gamma$ | 伽马 (gamma) | 可学习的缩放参数 | 神经网络训练时自己学出来的，初始值全是 1 |
| $\beta$ | 贝塔 (beta) | 可学习的偏移参数 | 也是学出来的，初始值全是 0 |

记忆方式：$\mu$ 和 $\sigma^2$ 是统计量（从数据算出来），$\gamma$ 和 $\beta$ 是参数（训练学出来），$\epsilon$ 是工程保底（写死的）。RMSNorm 删掉的全部内容，就是这张表里的一半。
