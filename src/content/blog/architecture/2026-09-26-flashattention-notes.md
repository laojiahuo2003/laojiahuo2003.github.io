...
description: FlashAttention 的全部机制只有三条：分块做 online softmax（前向不造 N×N 矩阵）、反向重算（不存 S 和 P）、单 kernel 融合（一次读写）。这篇把三条逐一拆开，用 2 行 2 列的手算例子走通 rescale 公式，再给一段可跑的 PyTorch 代码逐行认领，最后算清 HBM 读写为什么能从 Θ(N²) 降到 Θ(N)。
...
意思是：拿到第二段的新最大值后，把第一段的和按「旧最大值与新最大值的差」打折，再相加。这就是在线更新的全部数学——**每一块只需要自己局部的统计量，合并时旧块整体缩水**。输出 $O$ 的合并是同一套逻辑，多乘一个 $V$：
...
**手算一遍。** 取 $N=2,\ d=1$，$q=[1,2]$，$k=[1,3]$，$v=[5,10]$。先把直接 softmax 的答案摆出来对账：$S=[[1,3],[2,6]]$，$P=[[0.1192,0.8808],[0.0180,0.9820]]$，$O=PV=[9.404,\ 9.910]$。

块大小 $B_c=1$，两个 key 各一块，走一遍分块流程：

| 步骤 | $m$ | $\ell$ | $O$（未除以 $\ell$） |
|---|---|---|---|
| 初始 | $(-\infty, -\infty)$ | $(0, 0)$ | $(0, 0)$ |
| 块 1（$k_1{=}1,\ v_1{=}5$）：$S=[1,2]$，$\tilde m=[1,2]$，$\tilde P=[1,1]$ | $[1,2]$ | $[1,1]$ | $[5,5]$ |
| 块 2（$k_2{=}3,\ v_2{=}10$）：$\tilde m=[3,6]$，$\tilde P=[1,1]$ | $[3,6]$ | $[1{+}e^{-2},\, 1{+}e^{-4}]$ | $(e^{-2}\cdot 5{+}10,\ e^{-4}\cdot 5{+}10)$ |
| 最后除以 $\ell$ | — | — | $[9.404,\ 9.910]$ ✓ |

第二块是理解的关键：旧最大值 $[1,2]$ 升到新最大值 $[3,6]$，旧块要整体打折 $e^{-2}=0.1353$、$e^{-4}=0.0183$——**旧的峰值没新的高，权重按差距缩水**；新块 $\tilde P=[1,1]$ 乘 $v_2=10$ 按原价加入。归一化后 $O_1=(0.6767+10)/(1+0.1353)=9.404$，$O_2=(0.0916+10)/(1+0.0183)=9.910$，与直接 softmax 完全一致 ✓。
...
用手算例子的数字跑一遍（输出经 PyTorch 2.5 实测）：

```python
>>> q = torch.tensor([[1.],[2.]]); k = torch.tensor([[1.],[3.]]); v = torch.tensor([[5.],[10.]])
>>> flashattention(q, k, v, Bc=1)
tensor([[9.4040],
        [9.9101]])   # 与直接 softmax 一致
```

随机规模对照（$N{=}64/128/100$，$d{=}16/32/24$，$B_c{=}8/16/7$ 三组，与 `torch.softmax(Q@K.T)@V` 比较）：最大误差 $\sim 10^{-6}$，差异只是浮点求和顺序不同——**分块算法是精确算法，不是近似**。

## 总结

- **① 分块**：softmax 一行只需 $(m,\ell)$ 两个统计量即可合并，块结果按最大值差打折再相加——$S$ 全程不出现在 HBM；
