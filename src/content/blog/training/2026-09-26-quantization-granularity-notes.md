...
description: 从「量化到底在干什么」讲起——用一行 5 个权重手把手走通 q = round(x/s)，中间补上对称 vs 非对称量化（零点 z 什么时候免费、什么时候刚需），再逐个拆开三种粒度：per-tensor 一把尺子量全张量、per-channel 每个输出通道一把、group-wise 每 128 个元素一把。为什么尺子切得越细越准、per-channel 为什么在硬件上几乎免费、4-bit 为什么必须切到组。
...
（顺带一句：完整的量化写法 $x \approx s(q - z)$ 还多一个零点 $z$，$z = 0$ 就是对称量化——**下一节专讲**。）

## 2. 对称 vs 非对称：多一个零点 z，图什么

§1 里用的其实是**对称量化**（Symmetric Quantization）——它默认了一件事：数据以 0 为中心。完整的量化写法还多一项：
...
$z = 0$ 就退化回 §1 的 $q = \mathrm{round}(x/s)$。**对称（Symmetric）与非对称（Asymmetric）其实是同一套「仿射量化」（Affine Quantization）公式的两端：$z = 0$ 叫对称，$z \neq 0$ 叫非对称。** $z$ 不是来添乱的，是用来解决「数据不以 0 为中心」这个问题的。

### 对称量化：数轴关于 0 对折

INT8 对称量化把数轴对折到 0 两边，255 格正负各半：
...
好处：公式最简、计算最省（下面算账）。坏处：**如果数据本身不对称，一半格子就白费了。**

### 非对称量化：把数轴挪到数据上

看一个典型场景——**ReLU 之后的激活，全部 ≥ 0**。假设范围是 $[0,\ 6]$。

对称量化：$s = \max|x| / 127 = 6/127$，量化结果只会落在 $0 \sim 127$——负数那一半永远空着，128 个可用格（约 $2^7$），**8-bit 实际只剩 7-bit 精度**。

非对称量化：量程取满 $[-128,\ 127]$ 全部 256 格——$s = (\max - \min)/255 = 6/255$，零点 $z = \mathrm{round}(-\min/s) - 128$（让 $\min$ 正好落到 $-128$；本例 $z = -128$）：

```text
数据 [0, 6]，两种量法：
   对称：   s = 6/127  → q 落在 0 ~ 127，128 格 ≈ 7 bit，一半空着
   非对称： s = 6/255  → q 落在 -128 ~ 127，256 格全压进 [0, 6]，格点密一倍
```

同样 8 bit，格点密一倍。**数据有多偏，对称量化就浪费多少；非对称用 $z$ 把数轴挪到数据身上。**

§1 的 $W$ 对称量法够用，是因为它最大 1.5、最小 −1.2，两边幅度接近；真正吃大亏的是这种单边数据。

**分工原则一句话：数据大致关于 0 对称 → 对称量化；单边/偏斜 → 非对称量化。**

### 代价：z 会溜进矩阵乘

非对称不是白嫖的。把 $W = s_w(W^q - z_w)$、$x = s_x(x^q - z_x)$ 代进 $y_i = \sum_j W_{ij} x_j$：

$$
y_i = s_w s_x \left( \sum_j W^q_{ij} x^q_j \;-\; z_x \sum_j W^q_{ij} \;-\; z_w \sum_j x^q_j \;+\; z_w z_x \right)
$$

对称（$z_w = z_x = 0$）只剩第一项，一遍整数 GEMM 搞定。非对称多出三个修正项，但贵贱不一样：

- $-z_x \sum_j W^q_{ij}$：权重是提前知道的，每行的整数和可以预先算好——一个**每行常数**，并进 bias 就完事，几乎免费。
- $-z_w \sum_j x^q_j$：要对激活做全列求和，**多一次额外归约**——这才是贵的那个。

**所以工程上的分工几乎是被这份账决定的：**

- **权重：几乎总用对称**（$z_w = 0$）。一来权重分布本来就对称（初始化均值 0，训练后也大致对称），$z_w = 0$ 不亏精度；二来直接砍掉最贵的那一项。
- **激活：常非对称**。ReLU/GELU 之后是单边分布，非对称把精度找回来，代价只是并入 bias 的每行常数。
- 于是「**权重对称 + 激活非对称**」成了最常见的组合；SmoothQuant（§8）把激活抹平成对称分布之后，才敢全对称——W8A8 只剩一条 GEMM。

### 和「粒度」的关系

$z$ 和 $s$ 一样按粒度共享：per-tensor 一个 $z$、group-wise 每组一个 $z$（§8 里 GPTQ 说的「每组一套 scale/zero-point」就是这个）。本文后面所有「scale 管多大一片」的讨论对 $z$ 同样成立——**一把尺子其实是一个 $(s, z)$ 对。**

## 3. Per-Tensor：整个 Tensor 一个 Scale

**Per-Tensor Quantization（逐张量量化）**：整个矩阵只算一个 $s$，所有元素共用。（从本节起只谈 $s$，$z$ 永远按同样粒度跟着走。）
...
## 4. Per-Channel：每个 Channel 一个 Scale
...
- **LLM.int8()**：激活按 token、权重按输出通道量化（都满足 §5 的口诀，scale 能提出求和号）；再把 outlier 所在的个位数隐藏维度抠出来单独走 FP16——「少数维度用高精度」，是与粒度正交的另一条路。
...
| scale | — | 量化步长/缩放因子 |
| zero-point | — | 零点 |
| 对称量化 | Symmetric Quantization | 零点固定为 0（z=0），适合以 0 为中心的分布 |
| 非对称量化 | Asymmetric Quantization | 也叫仿射量化（Affine），带零点 z，量程覆盖 [min, max] |
| outlier | — | 离群值 |
...
- 对称 vs 非对称：$z = 0$ 公式最省但要求数据以 0 为中心；单边数据（如 ReLU 后的激活）用非对称把 256 格全用上。权重几乎总对称、激活常非对称，是矩阵乘修正项的账算出来的。
